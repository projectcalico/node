...
func configureAndCheckIPAddressSubnets(ctx context.Context, cli client.Interface, node *libapi.Node, k8sNode *v1.Node, defInterfacesToExclude []string) bool {
	// Configure and verify the node IP addresses and subnets.
	checkConflicts, err := configureIPsAndSubnets(node, k8sNode, defInterfacesToExclude, autodetection.GetInterfaces)
	if err != nil {
...
func MonitorIPAddressSubnets() {
	...
	for {
		...
		updated := configureAndCheckIPAddressSubnets(ctx, cli, node, k8sNode, DEFAULT_INTERFACES_TO_EXCLUDE)
		if updated {
			// Apply the updated node resource.
...
func configureIPsAndSubnets(node *libapi.Node, k8sNode *v1.Node, defInterfacesToExclude []string, getInterfaces func([]string, []string, int) ([]autodetection.Interface, error)) (bool, error) {
	...
	if ipv4Env == "autodetect" || (ipv4Env == "" && node.Spec.BGP.IPv4Address == "") {
		...
		cidr := autodetection.AutoDetectCIDR(adm, 4, k8sNode, defInterfacesToExclude, getInterfaces)
		if cidr != nil {
	...
	if ipv6Env == "autodetect" {
		...
		cidr := autodetection.AutoDetectCIDR(adm, 6, k8sNode, defInterfacesToExclude, getInterfaces)
		if cidr != nil {
