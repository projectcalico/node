...
func configureAndCheckIPAddressSubnets(ctx context.Context, cli client.Interface, node *libapi.Node, k8sNode *v1.Node, defInterfacesToExclude []string) bool {
	...
	checkConflicts, err := configureIPsAndSubnets(node, k8sNode, defInterfacesToExclude)
	if err != nil {
...
func configureIPsAndSubnets(node *libapi.Node, k8sNode *v1.Node, defInterfacesToExclude []string) (bool, error) {
	...
	if ipv4Env == "autodetect" || (ipv4Env == "" && node.Spec.BGP.IPv4Address == "") {
		...
		cidr := autodetection.AutoDetectCIDR(adm, 4, k8sNode, defInterfacesToExclude)
		if cidr != nil {
		...
		if ipv4Env != "" {
			...
			ipv4CIDROrIP, err := autodetection.GetLocalCIDR(ipv4Env, 4, autodetection.GetInterfaces)
			if err != nil {
	...
	if ipv6Env == "autodetect" {
		...
		cidr := autodetection.AutoDetectCIDR(adm, 6, k8sNode, defInterfacesToExclude)
		if cidr != nil {
