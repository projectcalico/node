...
var _ = Describe("UT for Node IP assignment and conflict checking.", func() {
	...
	DescribeTable("Test variations on how IPs are detected.",
		func(node *libapi.Node, items []EnvItem, expected bool) {
			...
			check, err := configureIPsAndSubnets(node, &v1.Node{}, getDefaultInterfacesToExclude())
...
var _ = Describe("UT for autodetection method k8s-internal-ip", func() {
	...
	DescribeTable("Test variations on k8s-internal-ip",
		...
		Entry("Test with \"IP\" = autodetect ,IP_AUTODETECTION_METHOD = k8s-internal-ip. k8snode = nil", &libapi.Node{}, nil, []EnvItem{{"IP", "autodetect"}, {"IP_AUTODETECTION_METHOD", "kubernetes-internal-ip"}}, false),
		Entry("Test with \"IP\" = autodetect ,IP_AUTODETECTION_METHOD = k8s-internal-ip. k8snode = valid addr", &libapi.Node{}, makeK8sNode("192.168.1.10", ""), []EnvItem{{"IP", "autodetect"}, {"IP_AUTODETECTION_METHOD", "k8s-internal-ip"}}, false),
	)
})
