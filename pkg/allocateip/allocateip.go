	...
	if addr == "" {
		...
		if err == nil {
			if attr[ipam.AttributeType] == attrString && attr[ipam.AttributeNode] == nodename {
				...
				// No attributes means that this is an old address, assigned by code that didn't use allocation attributes. It might be
				// a pod address, or it might be a node tunnel address. The only way to tell is by the existence of a handle, since workload
				// addresses have always used a handle, whereas tunnel addresses didn't start using handles until the same time as they got
				// allocation attributes.
				if handle != nil {
					// Handle exists, so this address belongs to a workload. We need to assign a new one for the node,
					// but we shouldn't clean up the old address.
					logCtx.WithField("currentAddr", addr).Info("Current address is occupied, assign a new one")
				} else {
					// Handle does not exist. This is just an old tunnel address that comes from a time before we used handles
					// and allocation attributes. Attempt to reassign the same address, but now with metadata. It's possible
					// that someone else takes the address while we do this, in which case we'll just need to assign a new address.
