...
import (
	...
	"github.com/projectcalico/node/pkg/buildinfo"
...
func Run(daemon bool) {
	...
	if !daemon {
		// Not running as a daemon - so this is a single shot update.
		run(nodename, cfg, c)
		return
	}
	...
	// Either create a typha syncclient or a local syncer depending on configuration. Callback into teh reconciler to
...
func (r *reconciler) OnUpdates(updates []bapi.Update) {
	...
	if updated && r.inSync {
		// We have updated data. Trigger an reconciliation, but don't block if there is already an update pending.
		select {
		case r.ch <- struct{}{}:
		default:
		}
...
// run performs a single shot update of the tunnel IP allocations.
func run(nodename string, cfg *apiconfig.CalicoAPIConfig, c client.Interface) {
