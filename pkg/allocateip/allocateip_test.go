...
var _ = Describe("Running as daemon", func() {
	...
	It("should handle adding and removing the tunnel IP after config updates", func() {
		node := makeNode("192.168.0.1/24", "fdff:ffff:ffff:ffff:ffff::/80")
		node.Name = "test.node"
		node.Status.WireguardPublicKey = "abcde"
		...
		// Wireguard is assigned first, then IPIP.
		...
		Eventually(func() error { return checkTunnelAddressForNode(c, ipam.AttributeTypeIPIP, "test.node", "192.16.0.2") }, "5s", "200ms")
		...
		Eventually(func() error { return checkTunnelAddressEmpty(c, ipam.AttributeTypeIPIP, "test.node") }, "5s", "200ms")
		Eventually(func() error { return checkTunnelAddressForNode(c, ipam.AttributeTypeVXLAN, "test.node", "192.16.0.3") }, "5s", "200ms")
		expectTunnelAddressForNode(c, ipam.AttributeTypeWireguard, "test.node", "192.16.0.1")
		...
		expectTunnelAddressForNode(c, ipam.AttributeTypeVXLAN, "test.node", "192.16.0.3")
		...
		Eventually(func() error { return checkTunnelAddressEmpty(c, ipam.AttributeTypeWireguard, "test.node") }, "2s", "200ms")
		expectTunnelAddressEmpty(c, ipam.AttributeTypeIPIP, "test.node")
		expectTunnelAddressForNode(c, ipam.AttributeTypeVXLAN, "test.node", "192.16.0.3")
